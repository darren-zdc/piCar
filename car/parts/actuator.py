    ...
    def set_pulse(self, pulse):
        ...
        if self.output > 0:
            self.pgio.hardware_PWM(self.pin, self.freq,
                                   int(self.output if self.inverted == False else 1e6 - self.output))
...
class PWMSteering:
    ...
    def __init__(self,
        ...
        self.pulse = car.utils.map_range(0, self.LEFT_ANGLE, self.RIGHT_ANGLE,
    ...
    def run_threaded(self, angle):
        ...
        self.pulse = car.utils.map_range(angle,
...
class PWMThrottle:
    ...
    def run_threaded(self, throttle):
        if throttle > 0:
            self.pulse = car.utils.map_range(throttle, 0, self.MAX_THROTTLE,
                                            self.zero_pulse, self.max_pulse)
        else:
            self.pulse = car.utils.map_range(throttle, self.MIN_THROTTLE, 0,
...
#         from adafruit_motor import Adafruit_MotorHAT, Adafruit_DCMotor
#         import atexit
#
